 ...
 */
function connectToGodot(): Promise<void> {
 ...
 */
function formatClassDoc(doc: any): string {
  if (doc.error) {
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
  ...
  switch (name) {
    case "explain_class": {
      const className = args?.class_name as string || "Node";
    ...
    case "compare_classes": {
      const class1 = args?.class1 as string || "Node";
      const class2 = args?.class2 as string || "Control";
